...
// startDate is optional: a hire recorded in the recruiting tool already has
// one, so the distros get the date itself instead of being told to wait for it.
// Left off, the email is exactly what it has always been.
export async function notifyHiredDistros({ referral, scoring, startDate }) {
  const money = (c) => (c ? ` + $${c}` : "");
  await sendMail({
    ...
    text:
      ...
      (startDate
        ? `Start date: ${startDate} — set in recruiting when they were hired. Rewards run from there.`
