...
export const CANDIDATE_SIDE_STAGES = ["on_hold", "rejected", "offer_rejected"];
...
export const STAGE_LABEL = {
  ...
  on_hold: "On hold",
  rejected: "Rejected",
  // We passed on them vs. they turned us down — the same button until now,
  // which left the reports unable to tell the two apart.
  offer_rejected: "Offer Rejected",
};

export const INTERVIEW_TYPES = ["phone", "video", "in_person"];

// How a scheduled interview ends when it isn't scored: nobody turned up, or
// the candidate called it off. Either way it's closed, not still waiting.
export const INTERVIEW_CLOSED = ["no_show", "canceled"];

export const INTERVIEW_CLOSED_LABEL = {
  no_show: "No Show Interview",
  canceled: "Candidate Canceled Interview",
};
...
export const ITEM_LABEL = {
  graded: "Graded 1–10",
  yesno: "Yes / No",
  // Stored as "note" since the very first form was saved; only the label the
  // team sees changed, so nothing needs migrating.
  note: "Custom Question",
};
  ...
  const sections = (form.sections || []).map((s) => ({
    ...
    items: (s.items || []).map((it) => {
      ...
      if (it.type === "yesno") {
        ...
        // Every question takes an optional note, a yes/no included — "no,
        // but only until October" is the kind of thing that decides a hire.
        return { type: "yesno", text: it.text || "", answer, note: String(given.note || "").trim() };
      }
...
export const DEFAULT_ROLES = {
  ...
  "Contract Review": ["Contract Review Representative"],
};
...
export function seatState(candidates, target) {
  const live = candidates.filter((c) => !["rejected", "offer_rejected"].includes(c.stage));
