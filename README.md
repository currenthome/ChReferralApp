...
- **Pipeline** (`/recruiting/pipeline`) — every active candidate by stage: Interviewing → Offer extended → Offer accepted → Hired. On hold, Offer Rejected and Rejected sit off to the side. Open anyone to schedule an interview, move them, or hire them.
- **Rejected vs. Offer Rejected** — two different things, and the reports need to tell them apart. *Rejected* is us passing on someone. *Offer Rejected* is them turning us down, and it's offered from Offer extended and from Offer accepted, since people also back out after saying yes. Neither holds a seat in a class, and either can be reopened back to Interviewing — people come back.
...
- **Entering past hiring** — the class list on that form covers every class, split into *Coming up* and *Already run* (most recent first, with the year on anything from another one). A class that has already run is still a class people were hired into, so months of past hiring can be entered against the real one rather than a stand-in. Note that a backfilled hire matched to a referral that was never marked hired does everything a hire normally does: it advances that referral, emails payroll/recruiting/Skip, and the nightly job pays Start and Day 30 straight away because the dates have passed. That is deliberate — the referrer earned it — but expect the mail to arrive in a batch.
...
- **Moving an interview** — Reschedule opens the booking form on what is booked now and overwrites it: one interview that moved, not two that were booked. The old time isn't kept on the record, so the Activity screen is where "this has moved three times" is answered.
- **An interview that didn't happen** — a scheduled interview also closes out as *No Show Interview* or *Candidate Canceled Interview* instead of being scored, so it stops showing as still to come. Once closed it can't be scored or moved; book a new one.
