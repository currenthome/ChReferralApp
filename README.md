...
- **Adding a candidate** — one form, because it's one phone call: name, mobile, department, role, target class, resume, pre-screen notes, and the first interview. Candidates are only added after a pre-screen, which is how the team actually works.
- **Hiring** asks for the person's first day, pre-filled with their class date. Every retention milestone counts from that date, and it's the date the referral side is handed when referral matching is wired up.
- **Sub-roles per department** live in `config/roles` (editable by execs), falling back to the defaults in `lib/recruiting.js`.
- **Scorecards** (`/recruiting/scorecards`) — one form per department, role, and interview type. Build your own sections and add any mix of graded (1–10), yes/no, and note questions. Publishing locks the form; reopening it for edits only affects interviews scored from then on. Recruiters and execs build any department's forms; a department manager builds their own.
